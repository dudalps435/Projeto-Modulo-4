...
/**
 * Calcula o tempo restante (em texto) para a próxima atualização semanal (segunda-feira 00:00)
 */
export function tempoParaProximaAtualizacao() {
  const agora = new Date();
  const proximaSegunda = new Date(agora);
  const diasAteSegunda = (1 - agora.getDay() + 7) % 7 || 7;
  proximaSegunda.setDate(agora.getDate() + diasAteSegunda);
  proximaSegunda.setHours(0, 0, 0, 0);

  const diffMs = proximaSegunda - agora;
